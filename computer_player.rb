...
class ComputerPlayer < Player
  ...
  def winning_percentage
    ...
    # puts @board
    puts "#{(@name + "'s").colorize(:red)}: #{@hand.pocket.join(" ")}"
    ...
    puts "#{@name} loses against #{hands[:does_worse_than].length} hands out of #{total} against a player with a #{player_range} range"
